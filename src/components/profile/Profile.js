...
const Profile = (props) => {
  ...
  const UpdateBio = async (bio, accessToken) => {
    ...
    await fetch(`/api/me/update/bio`, {
      method: "PUT", headers: { accessToken: accessToken, "Content-Type": "application/json" }, body: JSON.stringify({
        Bio: bio
      })
    }).then((res) => {
      if (res.status === 200) {
  ...
  const AddWork = async (Position, CName, to, from, current, accessToken) => {
    const res = await fetch(`/api/me/update/currentwork`, {
      method: "PUT", headers: { accessToken: accessToken, "Content-Type": "application/json" }, body: JSON.stringify({
    ...
    <div className='profileContainer'>
      ...
      <div className='mainContainer' style={{ display: "flex", justifyContent: "space-between", width: "94%", margin: "auto" }}>
        <div className='profileLeftBar' >
          <ProfileLeft handleOpenModal={setEditProfile} project={userDetail?.project} username={userDetail?.name} id={userDetail._id} bio={userDetail?.aboutBio} cover={userDetail.cover} profile={userDetail?.img} shortbio={userDetail?.shortBio} followers={userDetail.followers?.length} following={userDetail.following?.length} workExperience={userDetail?.workExperience} Education={userDetail?.education} Skills={userDetail?.skills} company={userDetail?.company} email={userDetail?.email} />
          ...
          <div style={{ margin: "10px 10px", display: "flex", flexDirection: "column" }} >
            ...
            <div style={{}} className="workExp" >
              ...
              <div style={{ margin: "10px 10px", display: "flex", flexDirection: "flex" }} >
                ...
                <div>
                  <p className={styles.p} style={{ fontSize: "10px" }} >From</p>
                ...
                {!checked && <div>
                  <p className={styles.p} style={{ fontSize: "10px" }} >to</p>
            ...
            {userDetail?.skills?.map((i) => {
              ...
              return (
                <div style={{ display: "flex", overflow: "auto" }}>
                  <p className={styles.p} style={{ color: "#aaa", marginRight: "15px", fontSize: "12px" }}>
