...
const SinglePost = (props) => {
  ...
  let username, image=singlePost?.image, caption =singlePost?.title, comments, profile, time, date, userId=singlePost?.user, likeNo= singlePost?.like?.length, commentNo, postId=singlePost?._id;
  let params = useParams();
  let paramId = params.id;
  let navigate = useNavigate();
  useEffect(() => {
    const getPost = async()=>{
      const res = await axios.get(`/api/single/post/${paramId}`);
...
getuserimg();
 ...
 const removesavedPost = async()=>{
  ...
  useEffect(()=>{
    if(singlePost?.like?.includes(user_Id)){ 
      setLike(liked);
    }
    else {
      setLike(disliked);
    ...
    const DeletePost = async()=>{
      const res = await fetch(`/api/delete/post/${postId}`,{method:"DELETE",headers:{accessToken:accessToken}});
      const data = await res.status;
      if( data === 200 ){
  ...
  return (
    <div style={{width:"100%",display:'flex',flexDirection:'column',alignItems:'center'}}>
      <ToastContainer
            ...
            />
      <div className={`post ${style.postWidth}`}>
        <div className="post__top">
          ...
          <div style={{ position: "absolute", right: "10px" }} className='dropdown' >
            ...
            <div className='dropdown-content2 button-hover'>
                          ...
                          <span className='dropicon' onClick={()=>DeletePost(userId)} >Delete</span>
                          </div>}
                        </div>
          </div>
        </div>
        ...
        <div className="post__options">
          <div className="post__option" onClick={()=>{ handleLike(); handleNotification(1,postId);}}>
          ...
          <div className="post__option">
            ...
            {/* <p>Share</p> */}
             ...
             {isMobileOnly ? <p onClick={()=>sharePost("Share Post",`post by @${user?.name}`,`https://${process.env.REACT_APP_CLIENTPORT}/post/${postId}`)} >Share</p> : <ShareContent
            label={"Share post"}
            title={`Post by ${user?.name}`}
            text={`Freedlink Post`}
            url={`https://${process.env.REACT_APP_CLIENTPORT}/post/${postId}`}
             /> }
          </div>
        </div>
      </div>
